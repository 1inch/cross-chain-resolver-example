...
async function main(): Promise<void> {
    ...
    const AMOUNT = BigInt(process.env.MAKING_AMOUNT_WEI || process.env.AMOUNT || '2000000000000000000') // 2e18 to match NEAR
    const SAFETY = 0n
    // Pack timelocks: 7 stages * 32 bits each, deployedAt in top 32 bits (set by factory)
    // Stages: SrcWithdrawal(0), SrcPublicWithdrawal(32), SrcCancellation(64), SrcPublicCancellation(96), DstWithdrawal(128), DstPublicWithdrawal(160), DstCancellation(192)
    ...
    const SRC_CANCEL_TS = BigInt(Math.floor(Date.now() / 1000) + 60) // 1min in future (must be > dst cancellation)
    ...
    console.log('[Resolver] deployDst for order', orderHashHex, 'amount', AMOUNT.toString())
    const data = iface.encodeFunctionData('deployDst', [imm, SRC_CANCEL_TS])
    const tx = await wallet.sendTransaction({to: RESOLVER, data, value: AMOUNT + SAFETY, gasLimit: 500000n})
    console.log('[Resolver] Submitted tx', tx.hash)
    const rcpt = await tx.wait()
    console.log('[Resolver] Mined in', rcpt?.blockNumber)
}
